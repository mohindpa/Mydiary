...
function applySettings(){ const d=activeDiary(); d.settings.size ??= 20; document.body.dataset.paper=d.settings.paper; document.body.dataset.font=d.settings.font; document.documentElement.style.setProperty('--entry-size',`${d.settings.size}px`); $('#fontSize').value=d.settings.size; $('#inputMode').value=d.settings.mode; $('#fontChoice').value=d.settings.font; $('#paperChoice').querySelectorAll('button').forEach(b=>b.classList.toggle('selected',b.dataset.paper===d.settings.paper)); $('#modeExplainer').textContent=d.settings.mode==='manglish' ? 'Type Malayalam sounds in English letters. Choose the word you mean from the small suggestion card.' : 'Write naturally in English.'; }
